...
import { type Distribution, experimental } from 'aws-cdk-lib/aws-cloudfront';
...
export class HashnodeSemanticSearchAuthStack extends Stack {
  ...
  constructor(scope: Construct, id: string, props?: StackProps) {
    ...
    // Allow also to sign requests for the search function using the same mechanism
    authFunction.addToRolePolicy(
      new PolicyStatement({
        sid: 'AllowInvokeSearchFunctionUrl',
        effect: Effect.ALLOW,
        actions: ['lambda:InvokeFunctionUrl'],
        resources: [`arn:aws:lambda:*:${this.account}:function:*-searchFn*`],
        conditions: {
          StringEquals: { 'lambda:FunctionUrlAuthType': 'AWS_IAM' },
        },
      })
